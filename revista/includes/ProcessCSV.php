...
class ProcessCSV
{
    ...
    private function create_images($csv_file)
    {
        ...
        foreach ($this->urls as $i => $url) {
            ...
            // This is still printing some warnings on the screen. I'll need to investigate this further.
            if (!is_wp_error($attachment_id)) {
                ...
                // Why is this not working? At the moment I need to do break the process down into the steps above.
                //set_post_thumbnail( absint($this->ids[$i]), absint($attachment_id) );
            }

            // EXPERIMENTAL: Insere os metadados no banco de dados
            $this->table = new RevistaTable();
            $this->table->insert($this->ids[$i], $attachment_id, 'NULL');

            // TODO: create a method to log the process
        }
    }
}
